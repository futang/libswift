...
void Channel::AddUncleHashes(Datagram& dgram, bin64_t pos) {
	...
	while (pos != peak && ((NOW & 3) == 3
			|| !data_out_cap_.within(pos.parent()))
...
bin64_t Channel::ImposeHint() {
	...
	ack_in_.twist(twist);
	bin64_t my_pick = file().ack_out().find_filtered(ack_in_, bin64_t::ALL,
			binmap_t::FILLED);
...
void Channel::Send() {
	...
	if (is_established()) {
		...
		if(!peers_sent)
		if (pFile != NULL) {
			while (fgets(peer, sizeof peer, pFile) != NULL) /* read a line */
			{
				if(!peer_req_rvd)break;
				Address a = Address(peer);
		...
		if(this->transfer().start_tp<=0)
			this->transfer().start_tp=NOW;
		AddHandshake(dgram);
		AddHave(dgram);
		AddAck(dgram);
	}
	...
	//	fprintf(stdout,"%s #%u sent %ib %s:%x\n",
	//				tintstr(),id_,dgram.size(),peer().str(),peer_channel_id_);
...
void Channel::AddHint(Datagram& dgram) {
	...
	if (hint_out_size_ < plan_pck) {
		...
		bin64_t hint = transfer().picker().Pick(ack_in_, diff, NOW + plan_for
				* 2);
...
bin64_t Channel::AddData(Datagram& dgram) {
	...
	if (data_out_.size() < cwnd_ && last_data_out_time_ + send_interval_ <= NOW
			+ luft) {
		tosend = DequeueHint();
...
else		dprintf("%s #%u sendctrl wait cwnd %f data_out %i next %s\n",
...
bin64_t Channel::OnData(Datagram& dgram) { // TODO: HAVE NONE for corrupted data
	bin64_t pos = dgram.Pull32();
	uint8_t *data;
	const Address& addr = dgram.address();
	int length = dgram.Pull(&data, 1024);
	bool ok = (pos == bin64_t::NONE) || (!file().ack_out().get(pos)
			&& file().OfferData(pos, (char*) data, length));
	dprintf("%s #%u %cdata %s\n",tintstr(),id_,ok?'-':'!',pos.str());
	data_in_ = tintbin(NOW, bin64_t::NONE);
	if (!ok){
//		fprintf(stderr, "first request time=%lld (drop)received data from %s data length=%d time=%lld chunk=%s\n", this->transfer().start_tp,addr.str(),length,NOW,pos.str());
	if (transfer().pFile != NULL) {
			//fprintf(transfer().pFile, "first request time=%lld (drop)received data from %s data length=%d time=%lld chunk=%s\n", this->transfer().start_tp,addr.str(),length,NOW,pos.str());
			//fclose(pFile);
			//fprintf(transfer().pFile, "(droped) %s %d %lld chunk=%s\r\n", addr.str(),length,NOW-this->transfer().start_tp,pos.str());
	...
	if (transfer().pFile != NULL) {
//		fprintf(transfer().pFile, "first request time=%lld received data from %s data length=%d time=%lld\n", this->transfer().start_tp,addr.str(),length,NOW);
		fprintf(transfer().pFile, "%s %d %lld chunk=%s\r\n", addr.str(),length,NOW-this->transfer().start_tp,pos.str());
				//fclose(pFile);
	}
	//fprintf(stderr, "first request time=%lld received data from %s data length=%d time=%lld\n", this->transfer().start_tp,addr.str(),length,NOW);
	return pos;
}
...
void Channel::TimeoutDataOut() {
	...
	while (!data_out_.empty() && (data_out_.front().time < timeout
			|| data_out_.front() == tintbin())) {
		if (data_out_.front() != tintbin() && ack_in_.is_empty(
				data_out_.front().bin)) {
			ack_not_rcvd_recent_++;
			data_out_cap_ = bin64_t::ALL;
	...
	while (!data_out_tmo_.empty() && data_out_tmo_.front().time < NOW
			- MAX_POSSIBLE_RTT)
...
void Channel::AddPexReq(Datagram& dgram) {
	if(!transfer().peerSelector->IsNeedPeer())return;
...
//	fprintf(stderr, "received data from %s\n", addr.str());
	...
	if (mych == 0) { // handshake initiated
		...
		for (binqueue::iterator i = file->hs_in_.begin(); i
				!= file->hs_in_.end(); i++)
...
void Channel::Reschedule() {
	...
	if (next_send_time_ != TINT_NEVER) {
		...
		dprintf("%s #%u closed\n",tintstr(),id_);
		transfer().peerSelector->DelPeer(this->peer(), transfer().root_hash());
		delete this;
	}
}
